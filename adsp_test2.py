...
questions = [
    ...
    데이터 분석을 하던 중 일부 데이터가 비어있음을 발견했다. 
    R에서 결측값을 확인하고자 한다. 다음 R 코드의 실행 결과로 올바른 것은 무엇인가?
    x <- c(1, 2, NA, 4, NA)
    is.na(x)""",
    ...
    다음과 같이 결측값이 포함된 데이터프레임이 있다. 
    분석 전에 결측값이 있는 행을 제거하고자 한다.
    ...
    설문조사 결과 일부 응답이 결측값으로 나타났다. 
    분석을 위해 결측값을 해당 변수의 평균값으로 대체하려고 한다.
    이 방법은 무엇이라 하는가?""",
        "code": None,
    ...
    어떤 데이터가 기하평균 ± 2.5표준편차 범위를 벗어나면 분석 목적에 맞지 않는 극단값일 수 있다.
    이 방법으로 데이터를 판단할 때 해당 값은 무엇으로 분류되는가?""",
        "code": None,
    ...
    데이터에 극단값이 포함되어 분석 결과가 왜곡될 가능성이 있다.
    극단값을 삭제하지 않고 상한값이나 하한값으로 조정하여 분석에 포함시키는 방법은 무엇인가?""",
        "code": None,
    ...
    한 연구에서 전국 초등학생 10만 명을 대상으로 조사를 하는데, 일부 1,000명을 선정하여 설문을 진행하였다.
    이때 모집단과 표본에 대한 설명으로 올바른 것은 무엇인가?""",
        "code": None,
        "options": [
            "표본 평균은 모집단 모수이다.",
            "모집단은 연구대상 전체, 표본은 일부이다.",
            "표본값은 모집단 통계량이다.",
            "모집단과 표본은 항상 동일하다."
        ],
        "answer": 1,
    ...
    모집단이 1,000명이고, 이 중 100명을 무작위로 선택하여 분석하고자 한다.
    이때 각 대상자가 선택될 확률이 동일하며, 복원추출 또는 비복원추출을 선택할 수 있는 방법은 무엇인가?""",
        "code": None,
    ...
    },
    {
        "week": 2,
        "question": """문제 8 – 계통추출법
    100명의 모집단에서 10명을 표본으로 뽑으려 한다. 계통추출법을 사용하면 K = N/n = 10이다.
    첫 번째 사람을 1번으로 선택한다면, 표본에 포함되는 사람은 누구인가?""",
        "code": None,
    ...
    },
    {
        "week": 2,
        "question": """문제 9 – 집락추출법
    대도시 5개 구를 집락으로 나누고, 각 구를 단위로 표본을 추출하는 방법은 무엇인가?""",
        "code": None,
    ...
    모집단이 성별·연령별로 다양하게 구성되어 있어, 각 층에서 표본을 뽑아 전체 대표성을 확보하려 한다.
    사용할 적절한 표본추출 방법은 무엇인가?""",
        "code": None,
        "options": ["단순랜덤추출", "계통추출", "집락추출", "층화추출"],
        "answer": 3,
